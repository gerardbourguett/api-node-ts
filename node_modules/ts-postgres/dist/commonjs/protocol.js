...
exports.Writer = exports.Reader = exports.writeMessage = exports.readRowData = exports.readRowDescription = exports.DatabaseError = exports.SegmentType = exports.TransactionStatus = exports.SSLResponseCode = exports.Message = exports.ErrorLevel = exports.SASL = exports.Command = void 0;
...
})(SSLResponseCode || (exports.SSLResponseCode = SSLResponseCode = {}));
...
const nullBuffer = node_buffer_1.Buffer.from('null');
...
function parseUuid(uuid) {
    return node_buffer_1.Buffer.from(uuid.replace(/-/g, ''), 'hex');
}
function makeBuffer(s, encoding, nullTerminate = false) {
    return node_buffer_1.Buffer.from(nullTerminate ? s + '\0' : s, encoding);
}
...
function readRowDescription(buffer, start, types) {
    ...
    while (i < length) {
        ...
        const innerDataType = types_js_1.arrayDataTypeMapping.get(dataType);
...
function readRowData(buffer, row, columnSpecification, encoding, bigints, types, streams) {
    ...
    while (i < columns) {
        ...
        if (start < end) {
            ...
            let skip = false;
            if (streams && spec === types_js_1.DataType.Bytea) {
                const stream = streams[j];
                if (stream) {
                    const slice = buffer.subarray(start, end);
                    const alloc = node_buffer_1.Buffer.allocUnsafe(slice.length);
            ...
            if (!skip) {
                ...
                if (isReader) {
                    ...
                    if (reader) {
                        value = reader(buffer, start, end, types_js_1.DataFormat.Binary, encoding);
                    }
                }
                else {
                    const read = (t, start, end) => {
                        ...
                        */
                        if (t >= types_js_1.DataType.MinUserOid)
                            return null;
                        switch (t) {
                            case types_js_1.DataType.Bool:
                                return (buffer[start] !== 0);
                            case types_js_1.DataType.Date: {
                            ...
                            case types_js_1.DataType.Timestamp:
                            case types_js_1.DataType.Timestamptz: {
                            ...
                            case types_js_1.DataType.Int2:
                                return buffer.readInt16BE(start);
                            case types_js_1.DataType.Int4:
                            case types_js_1.DataType.Oid:
                                return buffer.readInt32BE(start);
                            case types_js_1.DataType.Int8: {
                            ...
                            case types_js_1.DataType.Float4:
                                return buffer.readFloatBE(start);
                            case types_js_1.DataType.Float8:
                                return buffer.readDoubleBE(start);
                            case types_js_1.DataType.Bpchar:
                            case types_js_1.DataType.Char:
                            case types_js_1.DataType.Name:
                            case types_js_1.DataType.Text:
                            case types_js_1.DataType.Varchar:
                                return buffer.toString(encoding, start, end);
                            case types_js_1.DataType.Bytea:
                                const new_buffer = node_buffer_1.Buffer.allocUnsafe(end - start);
                                buffer.copy(new_buffer, 0, start, end);
                                return new_buffer;
                            case types_js_1.DataType.Jsonb:
                                if (buffer[start] === 1) {
...
function writeMessage(code, segments) {
    ...
    const buffer = node_buffer_1.Buffer.allocUnsafe(size);
...
function writeMessageInto(code, segments, buffer) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        switch (segment) {
            ...
            case SegmentType.Int64BE: {
                const n = value instanceof node_buffer_1.Buffer ? value.readBigInt64BE(0) : (typeof value === "bigint" ? value : BigInt(Number(value)));
...
class Writer {
    constructor(encoding) {
        ...
        this.outgoing = new buffer_js_1.ElasticBuffer();
    }
    bind(name, portal, format = types_js_1.DataFormat.Binary, values = [], types = []) {
            ...
            switch (dataType) {
                ...
                case types_js_1.DataType.Date: {
                    if (value === infinity) {
                        ...
                        size = add(SegmentType.Int32BE, (value.getTime() - timeshift) /
                            (1000 * 86400));
                    }
                    break;
                }
                ...
                case types_js_1.DataType.Timestamptz: {
                    if (value === infinity) {
                        size = (0, utils_js_1.sum)(add(SegmentType.UInt32BE, 0x7fffffff), add(SegmentType.UInt32BE, 0xffffffff));
                    }
                    else if (value === -infinity) {
                        size = (0, utils_js_1.sum)(add(SegmentType.UInt32BE, 0x80000000), add(SegmentType.UInt32BE, 0x00000000));
                    }
                    else if (value instanceof Date) {
                        const n = (value.getTime() - timeshift) * 1000;
                        const f = Math.floor(n / 4294967296);
                        const r = n - f * 4294967296;
                        size = (0, utils_js_1.sum)(add(SegmentType.Int32BE, f), add(SegmentType.UInt32BE, r));
                    }
                    break;
                }
                case types_js_1.DataType.Bpchar:
                ...
                case types_js_1.DataType.Int8: {
                    size = add(SegmentType.Int64BE, value instanceof node_buffer_1.Buffer ? value.readBigInt64BE(0) : (typeof value === "bigint" ? value : Number(value)));
                    break;
                }
                case types_js_1.DataType.Point: {
                    if ((0, types_js_1.isPoint)(value)) {
                        size = (0, utils_js_1.sum)(add(SegmentType.Float8, value.x), add(SegmentType.Float8, value.y));
                    }
                    break;
                }
                case types_js_1.DataType.Jsonb:
        ...
        const addBinaryArray = (value, dataType) => {
            ...
            const go = (level, value) => {
                ...
                if (length === 0)
                    return;
                if (level === dimCount) {
                    bytes += (0, utils_js_1.sum)(add(SegmentType.Int32BE, length), add(SegmentType.Int32BE, 1));
                    dimCount++;
                }
                for (let i = 0; i < length; i++) {
                    const v = value[i];
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (format === types_js_1.DataFormat.Binary) {
                ...
                const size = (result instanceof Array) ?
                    (0, utils_js_1.sum)(...result.map((s) => add(SegmentType.Buffer, makeBuffer(s, this.encoding)))) :
                    add(SegmentType.Buffer, (result === null) ?
                        nullBuffer :
    ...
    password(text) {
        this.enqueue(Command.Password, [makeBufferSegment(text, this.encoding, true)]);
    }
    saslInitialResponse(mechanism, clientNonce) {
        if (mechanism !== 'SCRAM-SHA-256')
            return false;
        const response = node_buffer_1.Buffer.from('n,,n=*,r=' + clientNonce);
    ...
    saslResponse(data, password, clientNonce) {
        const [response, signature] = (0, sasl_js_1.sign)(data, password, clientNonce);
        this.enqueue(SASL.SASLResponse, [
