...
const DEFAULTS = new defaults_js_1.Defaults(node_process_1.env);
...
class Client {
    ...
    */
    constructor(config = {}) {
        ...
        this.error = false;
        this.clientNonce = (0, node_crypto_1.randomBytes)(18).toString('base64');
        this.serverSignature = null;
        this.expect = 5;
        this.stream = new node_net_1.Socket();
        this.mustDrain = false;
        this.activeRow = null;
        this.bindQueue = new queue_js_1.Queue();
        this.closeHandlerQueue = new queue_js_1.Queue();
        this.cleanupQueue = new queue_js_1.Queue();
        this.errorHandlerQueue = new queue_js_1.Queue();
        this.preFlightQueue = new queue_js_1.Queue();
        this.rowDescriptionQueue = new queue_js_1.Queue();
        this.parameterDescriptionQueue = new queue_js_1.Queue();
        ...
        this.writer = new protocol_js_1.Writer(this.encoding);
        ...
        this.stream.on('error', (error) => {
            if (this.connecting) {
                ...
                if (this.ending && error.errno ===
                    node_os_1.constants.errno.ECONNRESET)
                    return;
    ...
    startup() {
        const writer = new protocol_js_1.Writer(this.encoding);
        ...
        if (ssl !== SSLMode.Disable) {
            ...
            this.stream.once('data', (buffer) => {
                ...
                switch (code) {
                    ...
                    case protocol_js_1.SSLResponseCode.Supported:
                        break;
                    // Server does not support SSL connections.
                    case protocol_js_1.SSLResponseCode.NotSupported:
                ...
                const context = ssl.options ?
                    (0, node_tls_1.createSecureContext)(ssl.options) :
                    undefined;
                const options = {
                    socket: this.stream,
                ...
                const stream = (0, node_tls_1.connect)(options, () => startup(stream));
    ...
    receive() {
        ...
        this.stream.on('data', (newBuffer) => {
            ...
            if (buffer && remaining) {
                ...
                if (free < length) {
                    const tempBuf = node_buffer_1.Buffer.allocUnsafe(size);
    ...
    connect(host, port, connectionTimeout) {
        ...
        let p = this.events.connect.once().then((error) => {
            ...
            return {
                encrypted: this.stream instanceof node_tls_1.TLSSocket,
                ...
                descriptionHandler: (description) => {
                    ...
                    resolve({
                        ...
                        execute: (values, portal, format, streams) => {
                            const result = (0, result_js_1.makeResult)(query?.transform);
                            ...
                            this.bindAndExecute(info, {
                                ...
                                format: format || query.format || types_js_1.DataFormat.Binary,
                                values: values || [],
                                close: false
                            }, types || query.types);
    ...
    query(text, values) {
        ...
        if (values && values.length) {
            const name = (query?.name) || ((this.config.preparedStatementPrefix ||
                DEFAULTS.preparedStatementPrefix) + (this.nextPreparedStatementId++));
            ...
            this.preFlightQueue.push({
                ...
                bind: {
                    name: name,
                    portal: portal,
                    format: format || types_js_1.DataFormat.Binary,
                    values: values,
                    close: true
                }
            });
    ...
    parseError(buffer) {
        ...
        if (level && code && message) {
            return new protocol_js_1.DatabaseError(level, code, details ? `${message}: ${details}` : message);
        }
    ...
    handle(buffer, offset, size) {
        ...
        while (size >= this.expect + read) {
            ...
            if (mtype === protocol_js_1.Message.RowData) {
                ...
                while (true) {
                    ...
                    const reader = new protocol_js_1.Reader(buffer, startRowData, bytes + read);
            ...
            const start = frame + 5;
            switch (mtype) {
                case protocol_js_1.Message.Authentication: {
                    const writer = new protocol_js_1.Writer(this.encoding);
                    ...
                    switch (code) {
                        ...
                        case 5: {
                            ...
                            const shadow = (0, utils_js_1.md5)(`${password || DEFAULTS.password}` +
                                `${user || DEFAULTS.user}`);
                            writer.password(`md5${(0, utils_js_1.md5)(shadow, salt)}`);
                            break;
                        }
                        case 10: {
                            const reader = new protocol_js_1.Reader(buffer, start + 4);
                            const mechanisms = [];
                            while (true) {
                ...
                case protocol_js_1.Message.BackendKeyData: {
                    this.processId = buffer.readInt32BE(start);
                    this.secretKey = buffer.readInt32BE(start + 4);
                    break;
                }
                case protocol_js_1.Message.BindComplete: {
                ...
                case protocol_js_1.Message.NoData: {
                ...
                case protocol_js_1.Message.EmptyQueryResponse:
                case protocol_js_1.Message.CommandComplete: {
                    const info = this.activeDataHandlerInfo;
                    if (info) {
                ...
                case protocol_js_1.Message.CloseComplete: {
                ...
                case protocol_js_1.Message.ErrorResponse: {
                ...
                case protocol_js_1.Message.Notice: {
                    const notice = this.parseError(buffer.subarray(start, start + length));
                    this.events.notice.emit(notice);
                    break;
                }
                case protocol_js_1.Message.NotificationResponse: {
                    const reader = new protocol_js_1.Reader(buffer, start);
                ...
                case protocol_js_1.Message.ParseComplete: {
                    break;
                }
                case protocol_js_1.Message.ParameterDescription: {
                ...
                case protocol_js_1.Message.ParameterStatus: {
                    const reader = new protocol_js_1.Reader(buffer, start);
                ...
                case protocol_js_1.Message.ReadyForQuery: {
                    if (this.error) {
                        this.error = false;
                    }
                ...
                case protocol_js_1.Message.RowDescription: {
                    const preflight = this.preFlightQueue.shift();
                    const reader = new protocol_js_1.Reader(buffer, start);
